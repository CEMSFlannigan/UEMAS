...
% Wavelength
N = length(finDistArr);                                % Signal length
...
[background,a,it,ord,s,fct] = backcor(finDistArr,finLineSum);%,10,0.01,'sh');
...
if ~isempty(z)
    ...
    % Sort the data
    [finDistArr,i] = sort(finDistArr);
    finLineSum = finLineSum(i);
    z = z(i);
    
    % Plot the results
    figure;
    subplot(2,1,1);
    plot(finDistArr,finLineSum,'b',finDistArr,z,'r');
    xlabel('n');
    legend('Simulated spectrum','Background estimation');
    title(['Estimation with function ''' fct ''', order = ' num2str(ord) ' and threshold = ' num2str(s)]);
    subplot(2,1,2);
    plot(finDistArr,finLineSum-z','r');
    xlabel('n');
